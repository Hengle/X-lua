...
namespace NodeEditorFramework
{
    ...
    public static class NodeEditorInputSystem
    {
        ...
        public static void InitSystem(out List<KeyValuePair<EventHandlerAttribute, Delegate>> events,
            out List<KeyValuePair<HotkeyAttribute, Delegate>> hotkeys,
            out List<KeyValuePair<ContextEntryAttribute, MenuFunctionData>> entries,
            out List<KeyValuePair<ContextFillerAttribute, Delegate>> fillers)
        {
            events = new List<KeyValuePair<EventHandlerAttribute, Delegate>>();
            hotkeys = new List<KeyValuePair<HotkeyAttribute, Delegate>>();
            entries = new List<KeyValuePair<ContextEntryAttribute, MenuFunctionData>>();
            fillers = new List<KeyValuePair<ContextFillerAttribute, Delegate>>();
                        ...
                        foreach (object attr in method.GetCustomAttributes(true))
                        {
                            ...
                            { // Method is an eventHandler
                                ...
                                { // Method signature is correct, so we register this handler
                                    ...
                                    events.Add(new KeyValuePair<EventHandlerAttribute, Delegate>(attr as EventHandlerAttribute, actionDelegate));
                                }
                            }
                            ...
                            { // Method is an hotkeyHandler
                                ...
                                { // Method signature is correct, so we register this handler
                                    if (actionDelegate == null) actionDelegate = Delegate.CreateDelegate(typeof(Action<NodeEditorInputInfo>), method);
                                    hotkeys.Add(new KeyValuePair<HotkeyAttribute, Delegate>(attr as HotkeyAttribute, actionDelegate));
                                }
                            }
                            ...
                            { // Method is an contextEntry
                                ...
                                { // Method signature is correct, so we register this handler
                                    ...
                                    entries.Add(new KeyValuePair<ContextEntryAttribute, MenuFunctionData>(attr as ContextEntryAttribute, menuFunction));
                                }
                            }
                            ...
                            { // Method is an contextFiller
                                ...
                                { // Method signature is correct, so we register this handler
                                    ...
                                    fillers.Add(new KeyValuePair<ContextFillerAttribute, Delegate>(attr as ContextFillerAttribute, methodDel));
                                }
                            }
                        }
            ...
            events.Sort((handlerA, handlerB) => handlerA.Key.priority.CompareTo(handlerB.Key.priority));
            hotkeys.Sort((handlerA, handlerB) => handlerA.Key.priority.CompareTo(handlerB.Key.priority));

            eventHandlers = events;
            hotkeyHandlers = hotkeys;
            contextEntries = entries;
            contextFillers = fillers;
        }
        ...
        //{
        //    eventHandlers = NodeSystemSetting.Inst.EventHandlers;
        //    hotkeyHandlers = NodeSystemSetting.Inst.HotkeyHandlers;
        //    contextEntries = NodeSystemSetting.Inst.ContextEntries;
        //    contextFillers = NodeSystemSetting.Inst.ContextFillers;
        //}
