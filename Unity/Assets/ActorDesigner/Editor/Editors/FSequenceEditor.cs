...
namespace FluxEditor
{
    ...
    public class FSequenceEditor : FEditorList<FContainerEditor>
    {
        ...
        public void OnPlaymodeWillChange()
        {
            ...
            {
                List<FTrackEditor> trackEditors = container.Editors;

                for (int i = 0; i != trackEditors.Count; ++i)
                {
                    if (trackEditors[i].Track.HasCache)
                    {
                        trackEditors[i].Track.ClearCache();
                    }
                }
            }
        ...
        public void OnGUI()
        {
                ...
                if (!(EditorDragged is FContainerEditor) && !(EditorDragged is FEventEditor))
                {
                    ...
                    foreach (FContainerEditor containerEditor in Editors)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (typeOfOwner == typeof(FTrackEditor))
                                {
                                    if (trackEditor.GetGlobalRect().Contains(Event.current.mousePosition))
                                    {
                                        editorBelowMouse = trackEditor;
                                        break;
                                    }
                                }
                                else
            ...
            switch (Event.current.type)
            {
                ...
                case EventType.DragPerform:
                    ...
                    //    FTrack track = FTrack.Create();
                    //    containerEditor.Container.Add(track);

                    //    Undo.RegisterCreatedObjectUndo(track.gameObject, "create Timeline");
